...
## Variablen
length = len(U_1['CH3'])
num = 100                               # Anzahle der Punkte in den Linien = len(U_1)/intervall

## Mittelwertbildung der Spannung
new_step = np.linspace(1, length, num)  # 10 20 30 40...
i_old = 0
neu_U1, neu_U2, neu_n1, neu_n2, neu_I1, neu_I2, neu_M1, neu_M2 = [],[],[],[],[],[],[],[]
...
neu_Time_U1, neu_Time_U2, neu_Time_n1, neu_Time_n2, neu_Time_I1, neu_Time_I2 = [],[],[],[],[],[]
...
## Anstieg von Drehzahl
test = np.polyfit(neu_Time_n1[10:30], neu_n1[10:30], 1)
...
fig.legend(bbox_to_anchor=(0.65, 0.6), loc='upper left', borderaxespad=0)
...
print('Maximaler Strom: ' + str(max(I_1['CH1'])) + ' A')
print('Maximales Drehmoment: ' + str(max(neu_M2)) + ' Nm')
print('Anstieg von Drehzahl 1 im Linearen bereich: ' + str(test[0]) + ' rpm/s')